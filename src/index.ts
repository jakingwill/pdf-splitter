...
/**
 * Manifest entry for each submission (used in both ZIP and JSON modes)
 */
type ManifestEntry = {
  submission_id: string;
  fileName: string;
  pageCount: number;
  download_url?: string; // Only present in JSON mode
};
...
/**
 * API response for JSON mode (with download URLs)
 */
type JsonSplitResponse = {
  job_id: string;
  totalPages: number;
  submissionCount: number;
  results: ManifestEntry[];
...
/**
 * Job registry entry for tracking temp directories and R2 keys
 */
type JobEntry = {
  jobId: string;
  directory: string;
  createdAt: number;
  manifest: Manifest;
  r2Keys: string[]; // R2 storage keys for cleanup
};
...
function validatePageRange(
  ...
  if (!submission_id || submission_id.trim() === '') {
    throw new Error(
      `Range at index ${index}: submission_id cannot be empty`
    );
  }
  ...
  if (start_page < 1) {
    throw new Error(
      `Range "${submission_id}": start_page must be >= 1, got ${start_page}`
    );
  }
  ...
  if (end_page < start_page) {
    throw new Error(
      `Range "${submission_id}": end_page (${end_page}) must be >= start_page (${start_page})`
    );
  }
  ...
  if (start_page > totalPages) {
    throw new Error(
      `Range "${submission_id}": start_page (${start_page}) exceeds total pages (${totalPages})`
    );
  }

  if (end_page > totalPages) {
    throw new Error(
      `Range "${submission_id}": end_page (${end_page}) exceeds total pages (${totalPages})`
    );
  }
}
 ...
 */
async function splitPdfFromBuffer(
  pdfBuffer: Buffer,
  ranges: PageRange[],
  outputDir: string,
  jobId: string,
  uploadToR2Storage: boolean = true
): Promise<{ totalPages: number; results: SplitResult[] }> {
  ...
  for (const range of ranges) {
    ...
    console.log(
      `Processing ${submission_id}: pages ${start_page}-${end_page}`
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to write output PDF "${outputPath}": ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
...
app.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      try {
        ...
        return res.status(400).json({
          ...
          error: `Invalid ranges format: ${
            error instanceof Error ? error.message : String(error)
          }. Expected JSON array of {submission_id, start_page, end_page} objects.`,
        } as ErrorResponse);
      }
      ...
      const { totalPages, results } = await splitPdfFromBuffer(
        req.file.buffer,
        ranges,
        outputDir,
        jobId,
        !useZipFormat // Upload to R2 only in JSON mode
      );
      ...
      if (useZipFormat) {
        ...
        const response: JsonSplitResponse = {
          job_id: jobId,
          totalPages: manifest.totalPages,
          submissionCount: manifest.submissionCount,
        ...
        console.log(`✓ Files available at: ${r2Keys.length > 0 ? 'R2 (persistent)' : 'local (60 minutes)'}`);
        console.log(`✓ Files stored in R2: ${r2Keys.length} of ${results.length}\n`);
 ...
 */
app.get('/jobs/:jobId/:fileName', async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
app.get('/jobs/:jobId/manifest.json', async (req: Request, res: Response) => {
  try {
